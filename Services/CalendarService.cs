namespace GuildfordBsac.Web.Services
{
    ...
    public class CalendarService : ICalendarService, IAsyncDisposable
    {
        ...
        private readonly CachedFetcher<IReadOnlyList<CalendarModel>> _fetcher;
        ...
        public CalendarService(IGoogleCalendarClient calendarClient, IMemoryCache cache, IOptions<AppSettings> settings, ILogger<CalendarService> logger)
        {
            ...
            _logger = logger;
            _fetcher = new CachedFetcher<IReadOnlyList<CalendarModel>>(cache);
        }

        public async Task<IReadOnlyList<CalendarModel>> GetCalendarsAsync(int year, CancellationToken cancellationToken = default)
        {
            ...
            return await _fetcher.GetOrFetchAsync(freshKey, async ct =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (_cache.TryGetValue(staleKey, out IReadOnlyList<CalendarModel>? stale) && stale != null)
                    {
                    ...
                    return (Array.Empty<CalendarModel>(), TimeSpan.Zero);
                }
            }, cancellationToken) ?? Array.Empty<CalendarModel>();
        }
